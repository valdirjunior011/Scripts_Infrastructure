...
Foreach ($element in $Data) {
 ...
 # Create insert statement
    try{    
        ...
        $i = 0
        $com.CommandText = "INSERT INTO " + $table + " (" + [string]::Join(",", ($properties | ForEach-Object{$_.Name})) + ") VALUES (" + [string]::Join(",", ($properties | ForEach-Object{$i++; "@$i"})) + ")"  
        $com.Parameters.Clear() 
        $i = 0
        $clear = $properties | ForEach-Object{$i++; $com.Parameters.AddWithValue("@$i", $_.Value)}
        $com.ExecuteScalar()
